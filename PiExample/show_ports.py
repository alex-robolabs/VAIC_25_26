...
Diagnostic for figuring out which /dev/ttyACM* endpoint corresponds to
which device. Run on the Pi:

    python3 ~/VAIC_25_26/PiExample/show_ports.py
...
The V5 Brain's User Port endpoint only enumerates when a user program is
running on the Brain. If you see only GPS endpoints (or only a V5
Communications Port without a User Port), the V5 is either powered off
or idle — start the ai_demo program and re-run.
"""
